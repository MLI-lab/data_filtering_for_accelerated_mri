...
def get_free_cuda_devices(update_enabled: bool = True, min_gpu : float = 80, min_mem : float = 4000, rep : int = 2, wait_time : float = 0.1):
    ...
    #if update_enabled:
        #indices = _get_free_device_indices_nvidia_smi_rep(min_gpu=min_gpu, min_mem=min_mem, rep=rep, wait_time=wait_time)
        #old_env = os.environ.get("CUDA_VISIBLE_DEVICES", "")
        #os.environ["CUDA_VISIBLE_DEVICES"] = ",".join([str(i) for i in indices]) if len(indices) > 0 else ""
        #logging.info("Updated CUDA_VISIBLE_DEVICES from {} to {}".format(old_env, os.environ["CUDA_VISIBLE_DEVICES"]))
    #else:
        #logging.info("CUDA_VISIBLE_DEVICES not updated")
